...
const CartItem = ({ onContinueShopping }) => {
  ...
  let total=0; // Initialize total to hold the cumulative sum
   cart.forEach(item => {
    const quantity = item.quantity; // Extract the quantity
...
const handleContinueShopping = (e) => {
    onContinueShopping(e); // Call the passed function to continue shopping
  ...
  const handleDecrement = (item) => {
    ...
    dispatch(updateQuantity({ name: item.name, quantity: item.quantity - 1 }));
      ...
      if (window.confirm('Remove item from cart?')){
        dispatch(removeItem(item)); // Add this line to remove the item
      } else {
        alert('Quantity cannot be less than 1. You may remove the item instead.');
      }
    }
};
